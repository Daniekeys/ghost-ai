...
1. Check `ui-context.md` for the node color palette. 
 Each palette options includes: 
 - a node background color 
 - a matching text color 

 Reuse existing theme color if they already exist in the `global.css`. Otherwise, keep the palette in the canvas types/constants, such as `types/canvas.ts`. 
...
4. Selected nodes should visually reflect their active pair 
