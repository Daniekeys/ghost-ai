# Canvarch — Phase 3: Workspace + Canvas UI
...
**AI presence** (when `isAI: true`): special Canvarch indicator:
- Avatar: `background: var(--accent-ai-dim)`, `border: 2px solid var(--accent-ai)`
- Content: a small spinning sparkle icon SVG (or just `✦` character) in `var(--accent-ai-text)`
...
Tooltip on hover: "Canvarch (generating)" or user's name.
...
- Canvarch label with the indigo dot: `●` in `var(--accent-ai)` + "Canvarch" in `var(--accent-ai-text)`, 14px, weight 600
...
```
Example: "Canvarch generated 8 nodes and 6 connections"
...
- Large sparkle icon (40px, `var(--text-faint)`)
- "Ask Canvarch to design your system" — 14px, `var(--text-muted)`, centered
...
```
Text: "Canvarch is generating..." in `var(--accent-ai-text)` with spinning indicator.
...
States:
- **Thinking**: `background: var(--accent-ai-dim)`, spinning ring in `var(--accent-ai)`, text in `var(--accent-ai-text)`: "Canvarch is designing your architecture..."
...
Alex: "Both! Canvarch, add Redis cache to the current architecture"  — 45s ago
[System] Canvarch generated 2 nodes and 3 connections  — 30s ago
```
...
**Generated by Canvarch** · Jun 4, 2025 at 2:34 PM
